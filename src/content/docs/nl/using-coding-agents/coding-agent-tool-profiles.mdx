...
<EngineeringMode>
Tool profiles translate product differences into engineering consequences. Architecture teams must compare context loading, tool access, permissions, subagents, hooks and evidence capture. Design work defines which workflows each tool may support. Build work validates claims with real repository tasks. Operations track vendor changes and policy fit. The risk is choosing a coding agent by brand or benchmark instead of workflow control.
</EngineeringMode>
...
<RealExample>
A team comparing Codex, Claude Code and Gemini Code Assist can run the same bug-fix task through each tool. The useful comparison is not brand preference: check which instructions were loaded, which files changed, what permissions were requested, which tests ran and how clearly each tool explained residual risk.
</RealExample>
